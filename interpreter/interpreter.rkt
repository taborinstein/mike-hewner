...
(define-datatype proc-val proc-val? 
  ...
  [closure-proc (params (listof symbol?)) (body (listof expression?)) (env environment?)] ; Standard lambda procedures
  [closure-proc-2 (param symbol?) (body (listof expression?)) (env environment?)]) ; Arbitrary variable lambda procedure
       ...
       (cond
         ...
         [(or (eqv? (1st datum) 'let) (eqv? (1st datum) 'letrec) (eqv? (1st datum) 'let*))
          (cond
            ...
            [else
             (cond
               [(eqv? (1st datum) 'let) (if (symbol? (2nd datum))
               ...
               [(eqv? (1st datum) 'let*) (let*-exp (map lit-exp (map car (2nd datum)))
                                                   (map parse-exp (map cadr (2nd datum)))
                                                   (map parse-exp (cddr datum)))]
               [(eqv? (1st datum) 'letrec) (letrec-exp (map car (2nd datum))
                                                       (map (lambda (x) (cadr (cadr x))) (2nd datum))
                                                       (map (lambda (x) (list (parse-exp (caddr (cadr x))))) (2nd datum))
                                                       (map parse-exp (cddr datum)))])])]
         [(eqv? (1st datum) 'set!) (cond
  ...
  (lambda (sym)
    #;(cases environment 
        ...
        [recur-extended-env-record
         ...
         (error 'global-env "Achievement unlocked: How did we get here?")]) ; Don't get rid of this section, just in case
