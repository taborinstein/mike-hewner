...
(define apply-env
  (lambda (env sym) 
    (cases environment env 
      [empty-env-record ()      
                        (error 'env "variable ~s not found." sym)] ; IC-SUGGESTION - 1, Can can a custom apply-env-global that distinguishes empty environment case, 2. Pass a closure that customizes a lookup failure
      [extended-env-record (syms vals env)
